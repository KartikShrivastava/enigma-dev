...
class tsx_tileset_loader : public pugi::xml_tree_walker {
public:
  tsx_tileset_loader(buffers::resources::Background *bg, const fs::path& fPath) {
    background = bg;
    background->set_use_as_tileset(true);
    parentDirPath = fPath.parent_path().u8string()+"/";
  }
...
std::unique_ptr<buffers::Project> TMXFileFormat::LoadProject(const fs::path& fPath) const {
  ...
  auto root = game->mutable_root();
  root->set_name("tsxRootTest");

  auto background = root->mutable_folder()->add_children();
  background->set_name("tsxBackgroundTest");

  tsx_tileset_loader backgroud_walker(background->mutable_background(), fPath);
